...
% Problem parameters
Tmax = [1E+1 6.5E-3];                    % Maximum acceleration
...
plot(x(5,:), x(6,:), 'Color', [0 0.4470 0.7410], 'LineWidth', 1)
xlabel('$x$ [km]')
ylabel('$z$ [km]')
...
figure;
hold on
plot(tau, sum(abs(u(1:2,:)), 1), 'Color', [0 0.4470 0.7410], 'LineWidth', 1)
xlabel('$\theta$')
ylabel('$\|\mathbf{u}\|_1$ $\mathrm{[m/s^2]}$')
grid on;
% xticklabels(strrep(xticklabels, '-', '$-$'));
% yticklabels(strrep(yticklabels, '-', '$-$'));
xlim([nu_0 nu_f])
...
figure;
hold on
plot(tau, sum(abs(u(3:4,:)), 1), 'Color', [0 0.4470 0.7410], 'LineWidth', 1)
xlabel('$\theta$')
ylabel('$\|\mathbf{u}\|_1$ $\mathrm{[m/s^2]}$')
grid on;
% xticklabels(strrep(xticklabels, '-', '$-$'));
...
save RendezvousATVSolutionCheb.mat -mat
